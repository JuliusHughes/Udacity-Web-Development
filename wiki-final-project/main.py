...
class WikiPage(Handler):
    def get(self, pagename):
        ...
        if not page:
            ...
            self.render("wiki.html", title=page.title, content=page.content, loggedin=loggedin)
...
class EditPage(Handler):
    def get(self, editpage):
        ...
        if not page:
            self.render("_edit.html", content="", title=pagename)
        else:
            self.render("_edit.html", content=page.content, title=page.title)
    ...
    def post(self, *args):
        ...
        if not page:
            wiki_content = WikiEntry(title=title, content=content)
            wiki_content.put()
        else:
            page.title = title
...
class Signup(Handler):
    ...
    def post(self):
        ...
        user_id = 2000
        self.response.headers.add_header('set-cookie', 'user_id=%s' % str(user_id))
        ...
        for user in users:
            if user.user_name == user_name_login:
                ...
                if user.pw_hash == user_login_hash:
                    userid_cookie_hash = make_cookie_hash(str(user_id_login))
                    self.response.headers.add_header('set-cookie', 'user_id=%s' % str(userid_cookie_hash))
                    self.request.cookies.get('user_id', str(user_id_login))
                    login_found = 1
                else:
                    user_id_login += 1
